...
(new THREE.JSONLoader).load 'javascripts/blender-fbx-export.json',  (geometryWithBones, materials) ->
...
#    m = new THREE.MeshLambertMaterial(color: 0x00ff00)
...
#    object.materials.phong1.side = THREE.DoubleSide
#    handMesh = new THREE.SkinnedMesh( geometry, object.materials.phong1)
#    handMesh.useVertexTexture = false  # causes giant error: array size must be a positive integer
    # not having it causes ehre this.boneTexture undefined inside skinned Texture updateMatrixWorld
    handMesh.scale = new THREE.Vector3(1,1,1)

#    max = -Infinity
#    min = Infinity
#    for face in handMesh.geometry.faces
#      throw "face not face3" unless face.constructor == THREE.Face3
#      max = Math.max(max, face.a)
#      max = Math.max(max, face.b)
#      max = Math.max(max, face.c)
#      min = Math.min(min, face.a)
#      min = Math.min(min, face.b)
#      min = Math.min(min, face.c)
...
#    handMesh.geometry.uvsNeedUpdate = true;
#    handMesh.geometry.buffersNeedUpdate = true;
...
#          new THREE.Euler(leapHand.roll(), leapHand.direction[1], -leapHand.direction[0], 'YZX' )
#        ))
#
